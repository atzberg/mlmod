...
import os, pickle;
script_base_name = "gen_mlmod_qoi1";
...
# define force model
#
class ML_QoI_Model(torch.nn.Module):

  def __init__(self,**params):
    super(ML_QoI_Model, self).__init__()
...
a = 4.1;  
ml_qoi_model = ML_QoI_Model(a=a,num_dim=num_dim,
...
model_name='qoi1';
f = open('%s/QoI_%s_params.pickle'%(base_dir,model_name),'wb');
params_force = {
'model_type':'QoI_ML1',
...
nn = ml_qoi_model.nn;
z = torch.zeros((nn,1));
traced_ml_qoi_model = torch.jit.trace(model, (z))

torch_filename = '%s/QoI_%s.pt'%(base_dir,model_name);
print("torch_filename = " + torch_filename);
traced_ml_qoi_model.save(torch_filename);
print(traced_ml_qoi_model.code) # prints trace model code
...
f = open('%s/QoI_%s_test_data.pickle'%(base_dir,model_name),'wb');
