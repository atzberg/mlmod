...
if flag_force: 
  mlmod_params = {'model_type':'F_ML1',
                'model_data':{
                  'base_name':qoi_case,
                  'base_dir':'./F_ML1_' + qoi_case,
                  'mask_fix':'FINAL_INTEGRATE',
                  'mask_input':'X V F Type',
                  'F_filename':'F_' + qoi_case + '.pt',
                  }
               };
  filename_mlmod_params = 'F_ML1_' + qoi_case + '.mlmod_params';
  write_mlmod_params(filename_mlmod_params,mlmod_params);
  #Lc("fix F_ML1_" + qoi_case " all mlmod " + filename_mlmod_params);
...
flag_qoi=True;
if flag_qoi: 
  mlmod_params = {'model_type':'QoI_ML1',
                  'model_data':{
                    'base_name':qoi_case,
                    'base_dir':'./QoI_ML1_' + qoi_case,
                    'mask_fix':'INITIAL_INTEGRATE FINAL_INTEGRATE',
                    'skip_step':1,
                    'mask_input':'X V F Type',
                    'qoi_filename':'QoI_' + qoi_case + '.pt',
                  }
               };
  filename_mlmod_params = 'QoI_ML1_' + qoi_case + '.mlmod_params';
  write_mlmod_params(filename_mlmod_params,mlmod_params);
  #Lc("fix F_ML1_" + qoi_case " all mlmod " + filename_mlmod_params);
  qoi1_fix_id = "QoI_ML1_1"; 
  Lc("fix %s all mlmod "%qoi1_fix_id + filename_mlmod_params);
...
# show the qoi
Lc('print "extract the mlmod computed QoI into local LAMMPS variables for further calculations"') 
pp = "print \"qoi1 = ["
for i in range(0,num_atoms*num_dim):
    ii = i + 1; # base 1 indexing 
    Lc("variable qoi1_%d equal f_%s[%d][1]"%(ii,qoi1_fix_id,ii));  # f_fixID, f_fixID[i], f_fixID[i][j]
    #Lc("print ${q_%d_%d}"%(ii1,ii2))
    pp += "${qoi1_%d}"%(ii);

    if (i < num_atoms*num_dim - 1):
      pp += ", ";
    else:
      pp += "]\"";
...
# extract value from QoI fix 
# Note, LAMMPS uses base 0 in L.numpy.extract_fix, where i=nrow, j=ncol.
print("");
print("extract the mlmod computed QoI into numpy array for further calculations.");
qoi1_fix_array = [];
for i in range(0,num_atoms*num_dim):
  qoi1_fix_array.append([L.numpy.extract_fix(qoi1_fix_id, fstyle=lconst.LMP_STYLE_GLOBAL, ftype=lconst.LMP_TYPE_ARRAY, nrow=i, ncol=0)]);
qoi1_fix_array = np.array(qoi1_fix_array);

print("qoi1_fix_array.shape = " + str(qoi1_fix_array.shape));
print("qoi1_fix_array = \n" + str(qoi1_fix_array));
print("");
