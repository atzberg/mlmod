...
class M_ij_Model(torch.nn.Module):
  ...
  def forward(self, x):
    ...
    r_ij_sq = torch.sum(torch.pow(vec_r_ij,2));
    ...
    c1 = 3.0*a/(4.0*r_ij_p);
    M_ij = c1*(torch.eye(num_dim) + (torch.outer(vec_r_ij,vec_r_ij)/r_ij_p_sq));
    c2 = a*a*a/(2.0*r_ij_p_cub)
    M_ij = M_ij + c2*(torch.eye(num_dim) - (torch.outer(vec_r_ij,vec_r_ij)/r_ij_p_sq));
    M_ij = c0*M_ij;

    z = M_ij.flatten().unsqueeze(1);
