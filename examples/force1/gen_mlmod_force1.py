...
# define force model
#
class ML_Force_Model(torch.nn.Module):
  def __init__(self,**params):
    super(ML_Force_Model, self).__init__()
...
a = 4.1;  
ml_force_model = ML_Force_Model(a=a,num_dim=num_dim,
...
model_name='force1';
f = open('%s/F_%s_params.pickle'%(base_dir,model_name),'wb');
params_force = {
'model_type':'F_ML1',
...
#--
model = ml_force_model;
nn = ml_force_model.nn;
z = torch.zeros((nn,1));
traced_ml_force_model = torch.jit.trace(model, (z))

torch_filename = '%s/F_%s.pt'%(base_dir,model_name);
print("torch_filename = " + torch_filename);
traced_ml_force_model.save(torch_filename);
print(traced_ml_force_model.code) # prints trace model code
...
print("ml_force_model");
y = ml_force_model(z);
print("z.shape = " + str(z.shape));
print("y.shape = " + str(y.shape));

f = open('%s/F_%s_test_data.pickle'%(base_dir,model_name),'wb');
